<!--
Auto-generated by: https://github.com/threlte/threlte/tree/main/packages/gltf
Command: npx @threlte/gltf@1.0.0-next.0 ./ramp_inverse.glb -T -t --shadows -r /assets/basic-vehicle-controller/
-->
<script lang="ts">
  import type * as THREE from 'three'
  import { Group } from 'three'
  import { T, type Props, type Events, type Slots } from '@threlte/core'
  import { useGltf } from '@threlte/extras'

  type $$Props = Props<THREE.Group>
  type $$Events = Events<THREE.Group>
  type $$Slots = Slots<THREE.Group>

  export const ref = new Group()

  type GLTFResult = {
    nodes: {
      RampInverse: THREE.Mesh
    }
    materials: {
      Ramps: THREE.MeshStandardMaterial
    }
  }

  const gltf = useGltf<GLTFResult>(
    '/../../Users/grischaerbe/Documents/Projects/2022/threlte/apps/docs-next/public/assets/basic-vehicle-controller/ramp_inverse-transformed.glb',
    { useDraco: true }
  )
</script>

{#if $gltf}
  <T is={ref} {...$$restProps}>
    <T.Mesh
      castShadow
      receiveShadow
      geometry={$gltf.nodes.RampInverse.geometry}
      material={$gltf.materials.Ramps}
      rotation={[Math.PI / 2, 0, 0]}
    />

    <slot {ref} />
  </T>
{/if}
